...
int     size=0,now_len=0,end,len,ret;
char    *data=NULL;
	...
	*ret_buf=NULL;
	...
	do{
		...
		if(len<0){
			/* エラー */
			Syslog(LOG_ERR,"sock.c:RecvOneLine_2():RecvOneLine_1():%d\n",len);
			free(data);
			data=NULL;
			ret=-1;
			end=1;
...
fprintf(stderr,"len=%d,size=%d,limit=%d\n",len,size,RECV_ONE_LINE_2_ALLOC_LIMIT);
			...
			if(data==NULL){
				/* メモリー確保エラー */
				SyslogPerror(LOG_ERR,"sock.c:RecvOneLine_2():malloc or realloc or limit-over");
				ret=-1;
				end=1;
			}
			else{
